...
func main() {
	...
	var recDl = flag.Duration("recv", 0, "receive timeout (deadline until next Recv times out)")
	...
	for {
		...
		if len(msg.Header) < 8 {
			log.Fatalf("ill-formed header (%d) bytes, expecting 4B channel and 4B request ID", len(msg.Header))
		}

		copy(key[0:4], msg.Header[0:4])
		copy(key[4:], msg.Header[len(msg.Header)-4:])
		reqid := binary.BigEndian.Uint32(key[4:])
		if seen[key] {
			log.Printf("ignoring duplicate request #%d", reqid)
			continue
		}
		seen[key] = true

		log.Printf("server: received request #%d: %q, port: %s", reqid, string(msg.Body), Port2Str(msg.Port))
		if *crash {
