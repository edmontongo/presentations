...
func (r *rep) receiver(ep mangos.Endpoint) {
	...
	for {
		log.Printf("rep: waiting for request ...")
		m := ep.RecvMsg()
		if m == nil {
			log.Printf("rep: done receiving from endpoint #%d", ep.GetID())
			return
		}
		...
		log.Printf("rep: queueing %q request into read queue ...", string(m.Body))
		select {
		case rq <- m:
		case <-cq:
			m.Free()
...
func (r *rep) sender() {
	...
	for {
		...
		case pe.q <- m:
		default:
			log.Printf("rep: DROPPING message ...")
