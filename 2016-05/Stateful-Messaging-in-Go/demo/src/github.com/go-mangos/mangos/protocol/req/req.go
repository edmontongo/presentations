...
func (r *req) resender() {
	...
	for {
		...
		id := binary.BigEndian.Uint32(r.reqmsg.Header)
		log.Printf("req: REQ #%d retry triggered (intvl: %s)", id, r.retry)
		r.Lock()
		m := r.reqmsg
		if m == nil {
			r.Unlock()
			...
			log.Printf("req: retried REQ #%d EXPIRED", id)
			r.Unlock()
			continue
		}
		m = m.Dup()
...
func (r *req) receiver(ep mangos.Endpoint) {
	...
	for {
		log.Printf("req: waiting for reply ...")
		m := ep.RecvMsg()
		if m == nil {
			break
		}
...
func (r *req) sender(pe *reqEp) {
	...
	for {
		...
		select {
		case m = <-rq:
			log.Printf("req: sender got %s via resend queue", string(m.Body))
		case m = <-sq:
			log.Printf("req: sender got %s via send channel", string(m.Body))
		case <-cq:
			return
		case <-pe.cq:
			return
		...
		if m.Expired() {
			log.Printf("req: message expired")
		}
		log.Printf("req: sending request %q", string(m.Body))
		if err := pe.ep.SendMsg(m); err != nil {
			log.Printf("req: send failed (%q), requeueing", err)
			r.resend <- m
			break
		}
	}
...
func (r *req) SendHook(m *mangos.Message) bool {
	...
	if os.Getenv("DROPMESSAGE") != "" {
		log.Printf("dropping request before sending it")
		return false
	}
...
func (r *req) RecvHook(m *mangos.Message) bool {
	...
	if r.reqmsg == nil {
		log.Printf("req: received stray reply (reqmsg is nil)")
		return false
	}
	if binary.BigEndian.Uint32(m.Header) != r.reqid {
		log.Printf("req: received reply with unknown REQ-ID")
		return false
	}
	r.waker.Stop()
	r.reqmsg.Free()
	r.reqmsg = nil
	log.Printf("req: received reply for REQ #%d", r.reqid)
	r.sock.SetRecvError(mangos.ErrProtoState)
	return true
}
